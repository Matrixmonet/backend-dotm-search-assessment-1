...
def main():
    ...
    parser.add_argument('--dir', type=str, default=os.getcwd(),
                        help="path to directory to be searched. (Default = current working directory)")
    parser.add_argument('--search_str', type=str, default="$",
                        help="substring to search for. (Default = '$')")
    ...
    matched_string = 0  # index of matched string in matches tuple

    # Main search loop
    for filename in os.listdir(os.getcwd()):
        file_count += 1
        if filename.endswith(".dotm"):
            # dotm files are zipped files
            with ZipFile(filename).open("word/document.xml") as search:
                for i, word in enumerate(search):
                    # finds search_str surrounded by 20 characters on each side
                    r = re.search(r_search, word)
                    try:
